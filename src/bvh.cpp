...
BVHNode *BVHAccel::construct_bvh(const std::vector<Primitive*>& prims, size_t max_leaf_size) {
  ...
  // Initialize a new BVHNode (root) with that bounding box.
  ...
  if (prims.size() <= max_leaf_size) {
    ...
    while (prims1->size() == 0 || prims2->size() == 0) {
      ...
      int axis;
      if (x > y && x > z) axis = 0; // x is longest
      if (y > x && y > z) axis = 1; // y is longest
      if (z > x && z > y) axis = 2; // z is longest
      ...
      if (prims1->size() == 0) bbox.min[axis] = split[axis]; 
      else if (prims2->size() == 0) bbox.max[axis] = split[axis];
    }
